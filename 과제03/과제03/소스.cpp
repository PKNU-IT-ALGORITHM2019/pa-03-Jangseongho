...
double bubbleSort(int A[], int n) {
	start = clock();
	for (int last = n - 1; last >= 1; last--) {  //n-1 ¹ø  
		for (int i = 0; i < n - 1; i++) {
			if (A[i] > A[i + 1]) {
				swap(A, i, i + 1);
			}
...
void quickSort_Mid(int A[], int left, int right) { // ÇÇº¿ = Áß°£°ª  
	int i = left, j = right;
...
void quickSort_Random(int A[], int left, int right) { //ÇÇº¿ = ·£´ý°ª
	int x, p;
	if (left < right) {
		x = rand() % (right + 1);
		swap(A, x, right);
